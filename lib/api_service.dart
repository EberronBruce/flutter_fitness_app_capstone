...
class ApiService {
  final String apiKey =
      ''; // NOTE: API key removed for security. Add your own from Google Cloud Console if needed.
  ApiService() {
  ...
  Future<String> _initPromptStream(
    String height,
    String weight,
    String goal,
    String timeline,
    ...
    try {
      ...
      final generatedText =
          response.text ?? ''; // Provide a default empty string if null
  ...
  Future<String> getDummyRecommendations() async {
    return Future.value(
      'This is a dummy recommendation for diet and exercise plan.',
    );
  }
}
